...
<AccordionGroup>
  ...
  <Accordion title="Run Mode">
    If you've configured a run script in Settings, press **`⌘E`** to execute it in the bottom terminal.
...
<Tip>
  Want to keep the plan but scrap the current run? Press **`⌘S`** to convert the session back to a spec—this deletes the worktree and leaves the instructions ready to try again later.
</Tip>
