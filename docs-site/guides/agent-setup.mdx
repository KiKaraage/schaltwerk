...
<Steps>
  <Step title="Select agent tab">
    Choose from: Claude, Codex, OpenCode, Gemini, or Factory Droid
  </Step>
...
**Key points:**
- Variables are scoped per agent type (Claude doesn't see Codex vars, Factory Droid stays isolated)
...
<AccordionGroup>
  ...
  <Accordion title="Schaltwerk uses old agent version after upgrade">
    ...
    **Solution:**
    ...
    3. Select the agent tab (Claude, Codex, etc.)
