...
Schaltwerk ships with a Model Context Protocol (MCP) server so AI assistants (Claude, Codex, Factory Droid, etc.) can automate session management.
...
<Tabs>
  ...
  <Tab title="Factory Droid">
    ...
    Then restart Factory Droid to load the MCP server.
  </Tab>
