...
<Warning>
  **NEVER** cancel a session before merging unless you want to lose the work. Use `⌘S` to convert to a Spec if you want to pause instead.
</Warning>
...
Pause work on a session without losing progress:

<Steps>
  <Step title="Press ⌘S">
    Converts the running session to a Spec
  </Step>

  <Step title="Worktree is removed">
    Git worktree deleted but all commits preserved
  </Step>

  <Step title="Restart later">
    Select the spec and press `⌘N` to restart as a session
  </Step>
</Steps>
...
<Warning>
  **Don't:**
  ...
  - Cancel sessions before merging (use `⌘S` to pause)
