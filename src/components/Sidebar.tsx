...
export function Sidebar() {
    ...
    useKeyboardShortcuts({
        ...
        onSelectSession: (index: number) => {
            const sorted = sortSessions<EnrichedSession>(sessions)
            const s = sorted[index]
            if (s) void handleSelectSession(s)
        },
        ...
        onSelectPrevSession: () => {
            ...
            if (selection.kind !== 'session') {
                void handleSelectSession(sorted[0])
                return
            }
            const currentIndex = sorted.findIndex(s => s.info.session_id === selection.payload)
            const nextIndex = Math.max(0, currentIndex - 1)
            const s = sorted[nextIndex]
            if (s) void handleSelectSession(s)
        },
        onSelectNextSession: () => {
            const sorted = sortSessions<EnrichedSession>(sessions)
            ...
            const currentIndex = sorted.findIndex(s => s.info.session_id === selection.payload)
            const nextIndex = Math.min(sorted.length - 1, (currentIndex >= 0 ? currentIndex : 0) + 1)
            const s = sorted[nextIndex]
            if (s) void handleSelectSession(s)
        },
    })
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {isBlocked && (
                                        <span className="text-xs px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400">blocked</span>
                                    )}
                                </div>
                            </button>
                        )
