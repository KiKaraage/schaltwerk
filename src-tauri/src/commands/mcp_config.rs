...
mod client {
    ...
    pub fn configure_mcp(client: McpClient, project_path: &str, mcp_server_path: &str) -> Result<String, String> {
        match client {
    ...
    fn configure_mcp_codex(mcp_server_path: &str) -> Result<String, String> {
        ...
        if let Some(start) = content.find(section_header) {
            ...
            for (i, _) in content[start + section_header.len()..].match_indices('\n') {
                ...
                if content[pos..].starts_with('[') { end = pos; break; }
            }
        ...
        if let Some(dir) = config_path.parent() { fs::create_dir_all(dir).map_err(|e| format!("Failed to create Codex config dir: {e}"))?; }
        let mut f = fs::File::create(&config_path).map_err(|e| format!("Failed to write Codex config: {e}"))?;
        f.write_all(content.as_bytes()).map_err(|e| format!("Failed to write Codex config: {e}"))?;
    ...
    fn remove_mcp_codex() -> Result<String, String> {
        ...
        if !config_path.exists() { return Ok("Codex config not found".to_string()); }
        let mut content = fs::read_to_string(&config_path).map_err(|e| format!("Failed to read Codex config: {e}"))?;
        let section_header = "[mcp_servers.schaltwerk]\n";
        if let Some(start) = content.find(section_header) {
            let mut end = content.len();
            for (i, _) in content[start + section_header.len()..].match_indices('\n') {
            ...
            content.replace_range(start..end, "");
            fs::write(&config_path, content).map_err(|e| format!("Failed to update Codex config: {e}"))?;
    ...
    pub fn codex_config_path() -> Result<(PathBuf, bool), String> {
        ...
        let base = std::env::var("CODEX_HOME").map(PathBuf::from).unwrap_or_else(|_| home.join(".codex"));
...
#[tauri::command]
pub async fn get_mcp_status(project_path: String, client: Option<String>) -> Result<MCPStatus, String> {
...
#[tauri::command]
pub async fn configure_mcp_for_project(project_path: String, client: Option<String>) -> Result<String, String> {
...
#[tauri::command]
pub async fn remove_mcp_for_project(project_path: String, client: Option<String>) -> Result<String, String> {
