...
mod tests {
    ...
    async fn test_paste_and_submit_terminal_executes() {
        ...
        assert!(buffer.contains("echo"), "Buffer should contain the echoed command");
    ...
    async fn test_paste_and_submit_bracketed_paste_mode() {
        ...
        let buffer = manager.get_terminal_buffer(id.clone()).await.unwrap();
        assert!(buffer.contains("multi"), "Should contain multi-line content");
    ...
    async fn test_paste_and_submit_partial_failure_recovery() {
        ...
        let result = manager.paste_and_submit_terminal(
            closed_id, 
            b"test".to_vec()
        ).await;
        
        assert!(result.is_ok(), "Paste should succeed silently for non-existent terminals");
        
        // Test that paste works for existing terminals
        let existing_id = unique_id("existing-terminal");
        manager.create_terminal(existing_id.clone(), "/tmp".to_string()).await.unwrap();
        
        let result2 = manager.paste_and_submit_terminal(
            existing_id.clone(), 
            b"echo test".to_vec()
        ).await;
    ...
    async fn test_concurrent_terminal_creation() {
        ...
        for i in 0..num_terminals {
            ...
            let handle = tokio::spawn(async move {
                ...
                manager_clone.create_terminal(id.clone(), "/tmp".to_string()).await.unwrap();
    ...
    async fn test_resource_tracking_across_async_ops() {
        ...
        let resize_handle = tokio::spawn(async move {
            for size in [80, 120, 100, 90] {
                manager_clone2.resize_terminal(id_clone2.clone(), size, 24).await.unwrap();
    ...
    async fn test_environment_variable_handling() {
        ...
        manager.create_terminal_with_env(
            id.clone(), 
            "/tmp".to_string(), 
            custom_env
        ).await.unwrap();
        
        manager.write_terminal(id.clone(), b"echo $CUSTOM_VAR\n".to_vec()).await.unwrap();
        sleep(Duration::from_millis(200)).await;
    ...
    async fn test_shell_detection_and_configuration() {
        ...
        for (id, _expected_shell) in test_shells {
            ...
            manager.write_terminal(id.clone(), b"echo $SHELL\n".to_vec()).await.unwrap();
            sleep(Duration::from_millis(200)).await;
    ...
    async fn test_cleanup_on_panic() {
        ...
        manager.create_terminal(panic_id.clone(), "/tmp".to_string()).await.unwrap();
        assert!(manager.terminal_exists(&panic_id).await.unwrap());
        
        let manager_clone = Arc::clone(&manager);
        let panic_id_clone = panic_id.clone();
        let handle = tokio::spawn(async move {
            manager_clone.write_terminal(panic_id_clone, b"test".to_vec()).await.unwrap();
    ...
    async fn test_race_conditions_during_creation_destruction() {
        ...
        let result = timeout(Duration::from_secs(30), async {
            ...
            let create_handle = tokio::spawn(async move {
                for _ in 0..5 { // Reduced from 10 to 5 iterations
                    let _ = manager1.create_terminal(race_id_clone.clone(), "/tmp".to_string()).await;
            ...
            let check_handle = tokio::spawn(async move {
                for _ in 0..10 { // Reduced from 20 to 10 checks
    ...
    async fn test_timing_sensitive_operations() {
        ...
        manager.create_terminal(timing_id.clone(), "/tmp".to_string()).await.unwrap();
        ...
        for (data, delay_ms) in operations {
            manager.write_terminal(timing_id.clone(), data).await.unwrap();
            sleep(Duration::from_millis(delay_ms)).await;
        }
        
        let buffer = manager.get_terminal_buffer(timing_id.clone()).await.unwrap();
    ...
    async fn test_process_zombie_prevention() {
        ...
        for i in 0..3 {
            ...
            manager.write_terminal(id.clone(), b"sleep 1 &\n".to_vec()).await.unwrap();
            ids.push(id);
        }
    ...
    async fn test_signal_handling() {
        ...
        manager.create_terminal(signal_id.clone(), "/tmp".to_string()).await.unwrap();
        
        manager.write_terminal(
            signal_id.clone(), 
        ...
        manager.write_terminal(signal_id.clone(), b"sleep 100 &\n".to_vec()).await.unwrap();
        sleep(Duration::from_millis(100)).await;
    ...
    async fn test_terminal_with_custom_size() {
        ...
        manager.create_terminal_with_size(
            size_id.clone(), 
            "/tmp".to_string(), 
            120, 
            40
        ...
        manager.write_terminal(size_id.clone(), b"tput cols\n".to_vec()).await.unwrap();
        sleep(Duration::from_millis(200)).await;
    ...
    async fn test_terminal_with_custom_app() {
        ...
        let result = manager.create_terminal_with_app(
            app_id.clone(),
            "/tmp".to_string(),
            "/bin/sh".to_string(),
        ...
        assert!(result.is_ok(), "Should create terminal with custom app successfully");
    ...
    async fn test_path_environment_merging() {
        ...
        manager.write_terminal(path_id.clone(), b"echo $PATH\n".to_vec()).await.unwrap();
        sleep(Duration::from_millis(200)).await;
    ...
    async fn test_shell_specific_escaping() {
        ...
        manager.create_terminal(escape_id.clone(), "/tmp".to_string()).await.unwrap();
        ...
        for test_str in test_strings {
            manager.write_terminal(escape_id.clone(), test_str).await.unwrap();
            sleep(Duration::from_millis(50)).await;
        }
        
        let buffer = manager.get_terminal_buffer(escape_id.clone()).await.unwrap();
    ...
    async fn test_event_bridge_initialization_timing() {
        ...
        manager.create_terminal(event_id.clone(), "/tmp".to_string()).await.unwrap();
        let creation_time = start.elapsed();
        
        assert!(creation_time.as_millis() < 1000, "Terminal creation including event bridge should be fast");
    ...
    async fn test_app_handle_setting() {
        ...
        manager.create_terminal(test_id.clone(), "/tmp".to_string()).await.unwrap();
    ...
    async fn test_paste_multiline_with_special_chars() {
        ...
        manager.create_terminal(multiline_id.clone(), "/tmp".to_string()).await.unwrap();
        
        let complex_data = b"#!/bin/bash\necho \"Hello $USER\"\nfor i in {1..3}; do\n  echo \"Count: $i\"\ndone";
        
        manager.paste_and_submit_terminal(multiline_id.clone(), complex_data.to_vec()).await.unwrap();
        
        sleep(Duration::from_millis(300)).await;
        
        let buffer = manager.get_terminal_buffer(multiline_id.clone()).await.unwrap();
    ...
    async fn test_rapid_create_close_cycles() {
        ...
        for cycle in 0..3 { // Reduced cycles
    ...
    async fn test_error_propagation() {
        ...
        let result = manager.write_terminal(unique_id("non-existent"), b"test".to_vec()).await;
        assert!(result.is_ok(), "Write to non-existent terminal should succeed silently");
        
        // Resize operations silently succeed for non-existent terminals in LocalPtyAdapter
        let result = manager.resize_terminal(unique_id("non-existent"), 80, 24).await;
        assert!(result.is_ok(), "Resize of non-existent terminal should succeed silently");
        ...
        assert!(result.is_ok(), "Getting buffer of non-existent terminal should succeed with empty data");
        ...
        assert!(result.is_ok(), "Closing non-existent terminal should succeed silently");
    }
    ...
    async fn test_cleanup_idempotency() {
        ...
        manager.create_terminal(cleanup_id.clone(), "/tmp".to_string()).await.unwrap();
    ...
    async fn test_paste_empty_data() {
        ...
        manager.create_terminal(empty_id.clone(), "/tmp".to_string()).await.unwrap();
        
        let result = manager.paste_and_submit_terminal(empty_id.clone(), vec![]).await;
    ...
    async fn test_large_write_operations() {
        ...
        manager.create_terminal(large_id.clone(), "/tmp".to_string()).await.unwrap();
