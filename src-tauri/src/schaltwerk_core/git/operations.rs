...
mod tests {
    ...
    fn test_has_uncommitted_changes_with_staged() {
        ...
        // Test - should have changes
        let result = has_uncommitted_changes(temp_dir.path()).expect("Should check status");
        assert!(result, "Should detect staged changes as uncommitted changes");
    }
    ...
    fn test_commit_all_changes() {
        ...
        let has_changes = has_uncommitted_changes(temp_dir.path())
            .expect("Should check status");
        assert!(!has_changes, "Should have no uncommitted changes after commit");
    ...
    fn test_commit_all_changes_no_changes() {
        ...
        let head = repo.head().expect("Failed to get HEAD");
        let oid = head.target().expect("HEAD should have target");
        assert_eq!(oid, initial_commit, "HEAD should not have moved when there were no changes");
    }
    ...
    fn test_commit_all_changes_first_commit() {
        ...
        // Create a file
        fs::write(temp_dir.path().join("README.md"), "# Test Project").expect("Failed to write file");
        
        // Commit - should handle first commit case
        commit_all_changes(temp_dir.path(), "First commit")
            .expect("Should create first commit");
        ...
        assert_eq!(commit.parent_count(), 0, "First commit should have no parents");
    }
}